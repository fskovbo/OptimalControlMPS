...
std::vector<double> SeedGenerator::generateRange(double a, double b, double c)
{ //equiv to a:b:c from MATLAB
  std::vector<double> array;
  while(a <= c + 1e-7)
  {
      array.push_back(a);
...
std::vector<double> SeedGenerator::linsigmoidSeed(double u_start, double u_end, size_t length)
{
  ...
  // generate random coefficients for ramp function
  ...
  // build shape function maintaining fixed endpoints of seed
...
{
  // adiabatic-style ramp without any random coefficients 
...
std::vector<double> SeedGenerator::randomCoeffSeed(double min, double max, size_t N)
{
  std::vector<double> v;
